...
enum { UDN_COL, NAME_COL, INTERFACE_COL, NUM_COLS };

static GtkWidget *combobox;
static GtkTreeModel *model;
static GUPnPContextManager *context_manager;

static gboolean find_device(const gchar *udn, GtkTreeIter *iter) {
  ...
  do {
    ...
    gtk_tree_model_get(model, iter, UDN_COL, &tmp, -1);

    if (tmp != NULL && strcmp(tmp, udn) == 0) found = TRUE;
...
static gboolean check_required_actions(
    GUPnPServiceIntrospection *introspection) {
  if (gupnp_service_introspection_get_action(introspection, "CreateObject") ==
      NULL)
    return FALSE;
  if (gupnp_service_introspection_get_action(introspection, "ImportResource") ==
      NULL)
    return FALSE;
  return TRUE;
}

static void get_introspection_cb(GUPnPServiceInfo *service_info,
                                 GUPnPServiceIntrospection *introspection,
  ...
  if (introspection != NULL) {
     ...
     * are implemented.
     */
    if (!check_required_actions(introspection)) goto error;
  }

  udn = gupnp_device_info_get_udn(device_info);
  if (G_UNLIKELY(udn == NULL)) goto error;

  /* First check if the device is already added */
  if (find_device(udn, &iter)) goto error;

  name = gupnp_device_info_get_friendly_name(device_info);
  if (name == NULL) name = g_strdup(udn);

  context = gupnp_device_info_get_context(device_info);
  interface = gssdp_client_get_interface(GSSDP_CLIENT(context));

  gtk_list_store_insert_with_values(GTK_LIST_STORE(model), NULL, -1, UDN_COL,
                                    udn, NAME_COL, name, INTERFACE_COL,
                                    interface, -1);
...
static void device_proxy_available_cb(GUPnPControlPoint *cp,
  ...
  gupnp_service_info_get_introspection_async(info, get_introspection_cb,
                                             g_object_ref(proxy));
}
...
static gboolean init(NstPlugin *plugin) {
  ...
  if (upload_cmd == NULL) return FALSE;
  g_free(upload_cmd);
  ...
  store = gtk_list_store_new(NUM_COLS, G_TYPE_STRING, /* UDN  */
                             G_TYPE_STRING,           /* Name */
  ...
  gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(combobox), renderer, "text",
                                NAME_COL);

  return TRUE;
}

static GtkWidget *get_contacts_widget(NstPlugin *plugin) { return combobox; }
...
static gboolean send_files(NstPlugin *plugin, GtkWidget *contact_widget,
  ...
  gtk_tree_model_get(model, &iter, UDN_COL, &udn, INTERFACE_COL, &interface,
                     -1);
  ...
  if (ret == FALSE) {
    g_warning("Could not send files to MediaServer: %s", err->message);
    g_error_free(err);
  }
...
static NstPluginInfo plugin_info = {
    "folder-remote", "upnp", N_("UPnP Media Server"), NULL,
    CAJA_CAPS_NONE,  init,   get_contacts_widget,     NULL,
    send_files,      destroy};
