...
static void _foreach_contact(gpointer contact, gpointer user_data) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (existing_contact) {
      ...
      g_string_append(contact_str, ")");
      g_hash_table_insert(jid_table, contact_str->str, existing_contact);
      ...
      g_string_append(contact_str, ")");
      g_hash_table_insert(jid_table, contact_str->str, contact_table);
      g_string_free(contact_str, FALSE);
    } else {
 ...
 * on queries
 */
static void _handle_dbus_exception(GError *error,
                                   gboolean empty_list_messages) {
  if (error == NULL) {
 ...
 */
static gboolean _get_contacts(void) {
  GError *error;
  ...
  if (!dbus_g_proxy_call(
          proxy, "prefs_list", &error, G_TYPE_INVALID,
  ...
  for (account_iter = accounts; *account_iter; account_iter++) {
    ...
    if (!dbus_g_proxy_call(
            proxy, "list_contacts", &error, G_TYPE_STRING,
            account,        /* call arguments */
...
static void _set_pixbuf_from_status(const gchar *show, GdkPixbuf **pixbuf) {
  GString *pixbuf_path;
  GError *error;
...
static void _add_contact_to_model(gpointer key, gpointer value,
                                  gpointer user_data) {
  GtkTreeIter *iter;
  GtkListStore *store;
  GdkPixbuf *pixbuf;
 ...
 */
static gboolean add_gajim_contacts_to_model(GtkListStore *store) {
  if (!_get_contacts()) {
    return FALSE;
  }
...
static void show_error(const gchar *title, const gchar *message) {
  GtkWidget *dialog;
...
static gboolean send_files(NstPlugin *plugin, GtkWidget *contact_widget,
  ...
  if (strlen(send_to) != 0) {
    ...
    show_error(_("Sending file failed"), _("Recipient is missing."));
    return FALSE;
  }
  ...
  for (file_iter = file_list; file_iter != NULL; file_iter = file_iter->next) {
    ...
    if (account) {
      dbus_g_proxy_call(proxy, "send_file", &error, G_TYPE_STRING, file_path,
                        G_TYPE_STRING, jid, G_TYPE_STRING, account,
                        G_TYPE_INVALID, G_TYPE_INVALID);
    } else {
      dbus_g_proxy_call(proxy, "send_file", &error, G_TYPE_STRING, file_path,
                        G_TYPE_STRING, jid, G_TYPE_INVALID, G_TYPE_INVALID);
    }
    g_free(file_path);
    if (error != NULL) {
...
static NstPluginInfo plugin_info = {
    "im-jabber",    "gajim", N_("Instant Message (Gajim)"), NULL,
