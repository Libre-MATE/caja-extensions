...
static void cb_mount_removed(GVolumeMonitor *volume_monitor, GMount *mount,
                             NstPlugin *plugin) {
  GtkTreeIter iter;
  GtkListStore *store;
  gboolean b, found;
...
static void cb_mount_changed(GVolumeMonitor *volume_monitor, GMount *mount,
                             NstPlugin *plugin) {
  GtkTreeIter iter;
  gboolean b;
  GtkListStore *store;
  ...
  while (b) {
    ...
    if (m == mount) {
      ...
      gtk_list_store_set(store, &iter, NAME_COL, name, ICON_COL,
                         g_mount_get_icon(mount), -1);
      g_free(name);
      g_object_unref(m);
      break;
    }
...
static void cb_mount_added(GVolumeMonitor *volume_monitor, GMount *mount,
                           NstPlugin *plugin) {
  char *name;
  GtkTreeIter iter;
  GtkTreeModel *model;
  ...
  if (g_mount_is_shadowed(mount) != FALSE) return;
  ...
  gtk_list_store_set(GTK_LIST_STORE(model), &iter, NAME_COL, name, ICON_COL,
                     g_mount_get_icon(mount), MOUNT_COL, mount, -1);

  g_free(name);

  if (select_added != FALSE) gtk_combo_box_set_active(GTK_COMBO_BOX(cb), 0);
}
...
static GtkWidget *get_contacts_widget(NstPlugin *plugin) {
  ...
  store =
      gtk_list_store_new(NUM_COLS, G_TYPE_STRING, G_TYPE_ICON, G_TYPE_OBJECT);
  ...
  for (l = mounts; l != NULL; l = l->next) {
    ...
    gtk_list_store_set(store, &iter, NAME_COL, name, ICON_COL,
                       g_mount_get_icon(l->data), MOUNT_COL, l->data, -1);
    g_free(name);
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(cb), text_renderer, "text", 0,
                                 NULL);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(cb), icon_renderer, "gicon", 1,
                                 NULL);
  gtk_combo_box_set_active(GTK_COMBO_BOX(cb), 0);
...
static NstPluginInfo plugin_info = {"folder-remote",
                                    "folder-remote",
