...
enum { COL_ICON, COL_ALIAS, NUM_COLS };

/*
 * Print appropriate warnings when dbus raised error
 * on queries
 */
static void handle_dbus_exception(GError *error) {
  if (error == NULL) {
...
static gboolean init(NstPlugin *plugin) {
  ...
  proxy = dbus_g_proxy_new_for_name(connection, SERVICE, OBJ_PATH, INTERFACE);
  dbus_g_connection_unref(connection);
  if (proxy == NULL) return FALSE;

  error = NULL;
  if (!dbus_g_proxy_call(proxy, "PurpleAccountsGetAllActive", &error,
                         G_TYPE_INVALID, DBUS_TYPE_G_INT_ARRAY, &accounts,
                         G_TYPE_INVALID)) {
...
static GdkPixbuf *get_buddy_icon(int id) {
  GError *error;
  ...
  error = NULL;
  if (!dbus_g_proxy_call(proxy, "PurpleBuddyGetIcon", &error, G_TYPE_INT, id,
                         G_TYPE_INVALID, G_TYPE_INT, &icon, G_TYPE_INVALID)) {
    handle_dbus_exception(error);
  }
  if (icon) {
    if (!dbus_g_proxy_call(proxy, "PurpleBuddyIconGetFullPath", &error,
                           G_TYPE_INT, icon, G_TYPE_INVALID, G_TYPE_STRING,
                           &path, G_TYPE_INVALID)) {
      handle_dbus_exception(error);
    }
...
static void add_pidgin_contacts_to_model(GtkTreeStore *store, GtkTreeIter *iter,
                                         GtkTreeIter *parent) {
  GError *error;
  ...
  for (i = 0; i < accounts->len; i++) {
    ...
    error = NULL;
    if (!dbus_g_proxy_call(proxy, "PurpleFindBuddies", &error, G_TYPE_INT,
                           account, G_TYPE_STRING, NULL, G_TYPE_INVALID,
                           DBUS_TYPE_G_INT_ARRAY, &contacts_list,
                           G_TYPE_INVALID)) {
      handle_dbus_exception(error);
      continue;
    }
    for (j = 0; j < contacts_list->len; j++) {
      ...
      error = NULL;
      if (!dbus_g_proxy_call(proxy, "PurpleBuddyIsOnline", &error, G_TYPE_INT,
                             id, G_TYPE_INVALID, G_TYPE_INT, &online,
                             G_TYPE_INVALID)) {
        handle_dbus_exception(error);
        continue;
      }
      ...
      error = NULL;
      if (!dbus_g_proxy_call(proxy, "PurpleBuddyGetName", &error, G_TYPE_INT,
                             id, G_TYPE_INVALID, G_TYPE_STRING, &dat->name,
                             G_TYPE_INVALID)) {
        handle_dbus_exception(error);
        g_free(dat);
        continue;
      }
      if (!dbus_g_proxy_call(proxy, "PurpleBuddyGetAlias", &error, G_TYPE_INT,
                             id, G_TYPE_INVALID, G_TYPE_STRING, &dat->alias,
                             G_TYPE_INVALID)) {
        handle_dbus_exception(error);
      }
  ...
  while (g_hash_table_iter_next(&hiter, NULL, (gpointer)&contacts_group)) {
    ...
    gtk_tree_store_set(store, parent, COL_ICON, NULL, COL_ALIAS, dat->alias,
                       -1);
    ...
    for (i = 0; i < num_accounts; ++i) {
      ...
      gtk_tree_store_set(store, iter, COL_ICON, icon, COL_ALIAS, dat->alias,
                         -1);
    }
  }
}

static void customize(GtkCellLayout *cell_layout, GtkCellRenderer *cell,
...
static GtkWidget *get_contacts_widget(NstPlugin *plugin) {
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(cb), renderer, "pixbuf",
                                 COL_ICON, NULL);
  gtk_cell_layout_set_cell_data_func(GTK_CELL_LAYOUT(cb), renderer, customize,
                                     (gboolean *)FALSE, NULL);
  renderer = gtk_cell_renderer_text_new();
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(cb), renderer, TRUE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(cb), renderer, "text",
                                 COL_ALIAS, NULL);
...
static gboolean send_file(int account, const char *who, const char *filename) {
  GError *error;
  int connection;

  error = NULL;
  if (!dbus_g_proxy_call(proxy, "PurpleAccountGetConnection", &error,
                         G_TYPE_INT, account, G_TYPE_INVALID, G_TYPE_INT,
                         &connection, G_TYPE_INVALID)) {
    handle_dbus_exception(error);
    return FALSE;
  }
  ...
  error = NULL;
  if (!dbus_g_proxy_call(proxy, "ServSendFile", &error, G_TYPE_INT, connection,
                         G_TYPE_STRING, who, G_TYPE_STRING, filename,
...
static gboolean send_files(NstPlugin *plugin, GtkWidget *contact_widget,
  ...
  path = gtk_tree_model_get_path(
      GTK_TREE_MODEL(gtk_combo_box_get_model(GTK_COMBO_BOX(contact_widget))),
      &iter);
...
static void free_contact(ContactData *dat) {
  g_free(dat->name);
  g_free(dat->alias);
  g_free(dat);
}
...
static NstPluginInfo plugin_info = {"im",
                                    "pidgin",
