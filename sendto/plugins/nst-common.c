...
static gboolean copy_fobject(GFile *source, GFile *dst) {
  GFileEnumerator *en;
  GFileInfo *info;
  GError *err = NULL;
  char *file_name;
  ...
  if (g_file_query_file_type(source, G_FILE_QUERY_INFO_NONE, NULL) !=
  ...
  en = g_file_enumerate_children(source, "*", G_FILE_QUERY_INFO_NONE, NULL,
                                 NULL);
...
gboolean copy_files_to(GList *file_list, GFile *dest) {
  ...
  for (l = file_list; l != NULL; l = l->next) {
    ...
    if (copy_fobject(source, dest) == FALSE) retval = FALSE;
