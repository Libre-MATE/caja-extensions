...
static void destroy_dialog(GtkWidget *widget, gpointer data) {
  gtk_main_quit();
}

static char *get_filename_from_list(void) {
  GList *l;
  ...
  if (encoding != NULL && strcasecmp(encoding, "UTF-8") != 0) use_utf8 = FALSE;
  ...
  while (TRUE) {
    ...
    if (matches == TRUE && cur_char != '\0') {
      offset++;
      common_part = g_string_append_unichar(common_part, cur_char);
    } else {
      break;
    }
  }
...
static char *pack_files(NS_ui *ui) {
  char *engrampa_cmd;
  const char *filename;
  GList *l;
  GString *cmd, *tmp;
  ...
  tmp_dir =
      g_strdup_printf("%s/caja-sendto-%s", g_get_tmp_dir(), g_get_user_name());
  g_mkdir(tmp_dir, 0700);
  tmp_work_dir = g_strdup_printf("%s/caja-sendto-%s/%li", g_get_tmp_dir(),
                                 g_get_user_name(), time(NULL));
  g_mkdir(tmp_work_dir, 0700);
  g_free(tmp_dir);

  if (gtk_combo_box_get_active(GTK_COMBO_BOX(ui->pack_combobox)) != 0) {
    pack_type = gtk_combo_box_text_get_active_text(
        GTK_COMBO_BOX_TEXT(ui->pack_combobox));
  } else {
    pack_type = NULL;
  ...
  g_settings_set_int(
      settings, CAJA_SENDTO_LAST_COMPRESS,
      gtk_combo_box_get_active(GTK_COMBO_BOX(ui->pack_combobox)));

  cmd = g_string_new("");
  g_string_printf(cmd, "%s --add-to=\"%s/%s%s\"", engrampa_cmd, tmp_work_dir,
                  filename, pack_type);
  g_free(engrampa_cmd);
  ...
  tmp = g_string_new("");
  g_string_printf(tmp, "%s/%s%s", tmp_work_dir, filename, pack_type);
  g_free(pack_type);
  g_free(tmp_work_dir);
...
static gboolean status_label_clear(gpointer data) {
  NS_ui *ui = (NS_ui *)data;
...
static void send_button_cb(GtkWidget *widget, NS_ui *ui) {
  char *error;
  NstPlugin *p;
  GtkWidget *w;
  ...
  if (p->info->validate_destination != NULL) {
    ...
    if (p->info->validate_destination(p, w, &error) == FALSE) {
      ...
      g_free(message);
      ui->status_timeoutid = g_timeout_add_seconds(
          CAJA_SENDTO_STATUS_LABEL_TIMEOUT_SECONDS, status_label_clear, ui);
  ...
  g_settings_set_string(settings, CAJA_SENDTO_LAST_MEDIUM, p->info->id);
...
static void send_if_no_pack_cb(GtkWidget *widget, NS_ui *ui) {
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ui->pack_checkbutton))) {
...
static void toggle_pack_check(GtkWidget *widget, NS_ui *ui) {
  ...
  if (enabled) {
    ...
    filename = gtk_entry_get_text(GTK_ENTRY(ui->pack_entry));
    if (filename == NULL || *filename == '\0') send_enabled = FALSE;
  }

  gtk_widget_set_sensitive(ui->send_button, send_enabled);
}

static void option_changed(GtkComboBox *cb, NS_ui *ui) {
  GtkWidget *w;
  NstPlugin *p;
  ...
  if (has_dirs == FALSE || supports_dirs != FALSE) {
    ...
    toggle =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ui->pack_checkbutton));
...
static gboolean set_model_for_options_combobox(NS_ui *ui) {
  GdkPixbuf *pixbuf;
  GtkTreeIter iter;
  GtkListStore *model;
  GtkIconTheme *it;
  ...
  last_used = g_settings_get_string(settings, CAJA_SENDTO_LAST_MEDIUM);
  ...
  for (aux = plugin_list; aux; aux = aux->next) {
    ...
    gtk_list_store_set(
        model, &iter, COLUMN_ICON, pixbuf,
#ifdef ENABLE_NLS
        COLUMN_DESCRIPTION,
        g_dgettext(p->info->gettext_package, p->info->description),
#else
    ...
    if (last_used != NULL && !strcmp(last_used, p->info->id)) {
      option = i;
      last_used_support_dirs =
          (p->info->capabilities & CAJA_CAPS_SEND_DIRECTORIES);
    }
    i++;
  }
  g_free(last_used);
  ...
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(ui->options_combobox), renderer,
                             FALSE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(ui->options_combobox),
                                 renderer, "pixbuf", COLUMN_ICON, NULL);
...
static void pack_entry_changed_cb(GObject *object, GParamSpec *spec,
                                  NS_ui *ui) {
...
static void update_button_image(GtkSettings *gtk_settings, GParamSpec *spec,
                                GtkWidget *widget) {
...
static void caja_sendto_create_ui(void) {
  GtkBuilder *builder;
  GError *error = NULL;
  NS_ui *ui;
  ...
  if (gtk_builder_add_from_resource(
          builder, "/org/mate/caja/extensions/sendto/caja-sendto.ui", &error) ==
      0) {
  ...
  ui->hbox_contacts_ws =
      GTK_WIDGET(gtk_builder_get_object(builder, "hbox_contacts_widgets"));
  ui->send_to_label =
      GTK_WIDGET(gtk_builder_get_object(builder, "send_to_label"));
  ui->options_combobox =
      GTK_WIDGET(gtk_builder_get_object(builder, "options_combobox"));
  ui->dialog =
      GTK_WIDGET(gtk_builder_get_object(builder, "caja_sendto_dialog"));
  ui->cancel_button =
  ...
  ui->pack_combobox =
      GTK_WIDGET(gtk_builder_get_object(builder, "pack_combobox"));
  ui->pack_entry = GTK_WIDGET(gtk_builder_get_object(builder, "pack_entry"));
  ui->pack_checkbutton =
      GTK_WIDGET(gtk_builder_get_object(builder, "pack_checkbutton"));
  ui->status_box = GTK_WIDGET(gtk_builder_get_object(builder, "status_box"));
  ui->status_label =
      GTK_WIDGET(gtk_builder_get_object(builder, "status_label"));
  ui->status_image =
      GTK_WIDGET(gtk_builder_get_object(builder, "status_image"));
  ...
  gtk_combo_box_set_active(
      GTK_COMBO_BOX(ui->pack_combobox),
      g_settings_get_int(settings, CAJA_SENDTO_LAST_COMPRESS));
  ...
  if (one_file) {
    ...
    filepath = g_filename_from_uri((char *)file_list->data, NULL, NULL);

    if (filepath != NULL) filename = g_path_get_basename(filepath);
...
static void caja_sendto_plugin_dir_process(const char *plugindir) {
  ...
  if (dir == NULL) {
    g_warning("Can't open the plugins dir: %s",
    ...
    while ((item = g_dir_read_name(dir))) {
      if (g_str_has_suffix(item, SOEXT)) {
        ...
        if (!g_module_symbol(p->module, "nst_init_plugin",
                             (gpointer *)&nst_init_plugin)) {
...
static gboolean caja_sendto_plugin_init(void) {
...
static char *escape_ampersands_and_commas(const char *url) {
  int i;
  char *str, *ptr;
...
static void caja_sendto_init(void) {
  int i;
  ...
  for (i = 0; filenames != NULL && filenames[i] != NULL; i++) {
    ...
    if (g_file_test(filename, G_FILE_TEST_IS_DIR) != FALSE) has_dirs = TRUE;
...
int main(int argc, char **argv) {
  ...
  if (caja_sendto_plugin_init() == FALSE) {
    ...
    gtk_message_dialog_format_secondary_text(
        GTK_MESSAGE_DIALOG(error_dialog), _("Please verify your installation"));
