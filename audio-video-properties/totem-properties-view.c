...
static void update_general(TotemPropertiesView *props, const GstTagList *list) {
  struct {
    const char *tag_name;
    const char *widget;
  } items[] = {
  ...
  for (i = 0; i < G_N_ELEMENTS(items); i++) {
    ...
    if (gst_tag_list_get_string_index(list, items[i].tag_name, 0, &string) !=
        FALSE) {
      bacon_video_widget_properties_set_label(props->priv->props,
                                              items[i].widget, string);
      g_free(string);
    }
  }
  ...
  if (gst_tag_list_get_string(list, GST_TAG_COMMENT, &comment) ||
    ...
    bacon_video_widget_properties_set_label(props->priv->props, "comment",
                                            comment);
    g_free(comment);
  }
  ...
  if (gst_tag_list_get_date(list, GST_TAG_DATE, &date)) {
    ...
    g_date_free(date);
    bacon_video_widget_properties_set_label(props->priv->props, "year", string);
    g_free(string);
...
static void set_codec(TotemPropertiesView *props, GstDiscovererStreamInfo *info,
                      const char *widget) {
  GstCaps *caps;
  const char *nick;
  ...
  if (g_str_equal(nick, "audio") == FALSE &&
    ...
    bacon_video_widget_properties_set_label(props->priv->props, widget,
                                            _("N/A"));
    return;
  }
...
static void set_bitrate(TotemPropertiesView *props, guint bitrate,
                        const char *widget) {
  char *string;
  ...
  bacon_video_widget_properties_set_label(props->priv->props, widget, string);
  g_free(string);
}

static void update_video(TotemPropertiesView *props,
  ...
  bacon_video_widget_properties_set_label(props->priv->props, "dimensions",
                                          string);
  g_free(string);

  set_codec(props, (GstDiscovererStreamInfo *)info, "vcodec");
  set_bitrate(props, gst_discoverer_video_info_get_bitrate(info),
              "video_bitrate");
...
static void update_audio(TotemPropertiesView *props,
                         GstDiscovererAudioInfo *info) {
  guint samplerate, channels;

  set_codec(props, (GstDiscovererStreamInfo *)info, "acodec");

  set_bitrate(props, gst_discoverer_audio_info_get_bitrate(info),
              "audio_bitrate");
  ...
  if (samplerate) {
    ...
    bacon_video_widget_properties_set_label(props->priv->props, "samplerate",
                                            string);
    g_free(string);
  } else {
    bacon_video_widget_properties_set_label(props->priv->props, "samplerate",
  ...
  if (channels) {
    ...
    bacon_video_widget_properties_set_label(props->priv->props, "channels",
                                            string);
    g_free(string);
  } else {
    bacon_video_widget_properties_set_label(
        props->priv->props, "channels", C_("Number of audio channels", "N/A"));
  }
}

static void discovered_cb(GstDiscoverer *discoverer, GstDiscovererInfo *info,
                          GError *error, TotemPropertiesView *props) {
  ...
  if (error) {
    g_warning("Couldn't get information about '%s': %s",
              gst_discoverer_info_get_uri(info), error->message);
    return;
  }
  ...
  /* Widgets */
  bacon_video_widget_properties_set_has_type(props->priv->props, has_video,
                                             has_audio);
  ...
  /* Video and Audio */
  if (video_streams) update_video(props, video_streams->data);
  if (audio_streams) update_audio(props, audio_streams->data);

  gst_discoverer_stream_info_list_free(video_streams);
  gst_discoverer_stream_info_list_free(audio_streams);
}

static void totem_properties_view_init(TotemPropertiesView *props) {
  GError *err = NULL;
  ...
  g_signal_connect(props->priv->disco, "discovered", G_CALLBACK(discovered_cb),
                   props);
}
...
static void totem_properties_view_finalize(GObject *object) {
  ...
  if (props->priv != NULL) {
    if (props->priv->disco) {
      g_signal_handlers_disconnect_by_func(props->priv->disco, discovered_cb,
                                           props);
...
static void totem_properties_view_set_location(TotemPropertiesView *props,
                                               const char *location) {
  g_assert(TOTEM_IS_PROPERTIES_VIEW(props));

  if (props->priv->disco) gst_discoverer_stop(props->priv->disco);
  ...
  if (location != NULL && props->priv->disco != NULL) {
    ...
    if (gst_discoverer_discover_uri_async(props->priv->disco, location) ==
        FALSE) {
...
GtkWidget *totem_properties_view_new(const char *location, GtkWidget *label) {
