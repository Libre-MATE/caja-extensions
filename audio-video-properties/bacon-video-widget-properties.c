...
G_DEFINE_TYPE_WITH_PRIVATE(BaconVideoWidgetProperties,
                           bacon_video_widget_properties, GTK_TYPE_BOX)

static void bacon_video_widget_properties_class_init(
    BaconVideoWidgetPropertiesClass *klass) {
...
static void bacon_video_widget_properties_init(
  ...
  gtk_orientable_set_orientation(GTK_ORIENTABLE(props),
...
void bacon_video_widget_properties_reset(BaconVideoWidgetProperties *props) {
  ...
  /* Container */
  bacon_video_widget_properties_set_label(props, "container",
                                          C_("Media container", "Unknown"));

  /* Dimensions */
  bacon_video_widget_properties_set_label(props, "dimensions",
                                          C_("Dimensions", "N/A"));
  /* Video Codec */
...
static char *time_to_string_text(gint64 msecs) {
  ...
  hours = g_strdup_printf(
      g_dngettext(GETTEXT_PACKAGE, "%d hour", "%d hours", hour), hour);

  mins = g_strdup_printf(
      g_dngettext(GETTEXT_PACKAGE, "%d minute", "%d minutes", min), min);

  secs = g_strdup_printf(
...
void bacon_video_widget_properties_set_duration(
    BaconVideoWidgetProperties *props, int _time) {
  char *string;

  g_return_if_fail(props != NULL);
  g_return_if_fail(BACON_IS_VIDEO_WIDGET_PROPERTIES(props));

  if (_time == props->priv->time) return;
...
void bacon_video_widget_properties_set_has_type(
    BaconVideoWidgetProperties *props, gboolean has_video, gboolean has_audio) {
  GtkWidget *item;
...
void bacon_video_widget_properties_set_framerate(
    BaconVideoWidgetProperties *props, float framerate) {
  gchar *temp;
  ...
  if (framerate > 1.0) {
    temp = g_strdup_printf(
        g_dngettext(GETTEXT_PACKAGE, "%0.2f frame per second",
...
GtkWidget *bacon_video_widget_properties_new(void) {
  ...
  const char *labels[] = {
      ...
      "container_label", "dimensions_label", "vcodec_label",
  ...
  if (gtk_builder_add_from_resource(
          xml, "/org/mate/caja/extensions/totemproperties/ui/properties.ui",
          NULL) == 0) {
    g_object_unref(xml);
    return NULL;
  }

  props = BACON_VIDEO_WIDGET_PROPERTIES(
      g_object_new(BACON_TYPE_VIDEO_WIDGET_PROPERTIES, NULL));
  ...
  for (i = 0; i < G_N_ELEMENTS(labels); i++)
    gtk_size_group_add_widget(
        group, GTK_WIDGET(gtk_builder_get_object(xml, labels[i])));
