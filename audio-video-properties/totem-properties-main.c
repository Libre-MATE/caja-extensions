...
static GList *totem_properties_get_pages(CajaPropertyPageProvider *provider,
                                         GList *files);
...
static void totem_properties_plugin_register_type(GTypeModule *module) {
  ...
  tpp_type = g_type_module_register_type(module, G_TYPE_OBJECT,
                                         "TotemPropertiesPlugin", &info, 0);
  g_type_module_add_interface(module, tpp_type,
...
static gpointer init_backend(gpointer data) {
  gst_init(NULL, NULL);
