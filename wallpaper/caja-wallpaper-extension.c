...
static void set_wallpaper_callback(CajaMenuItem *item, gpointer user_data) {
  GList *files;
  GSettings *settings;
  CajaFileInfo *file;
  gchar *uri;
...
static gboolean is_image(CajaFileInfo *file) {
  gchar *mimeType;
  gboolean isImage;
...
static GList *caja_cwe_get_file_items(CajaMenuProvider *provider,
...
static void caja_cwe_menu_provider_iface_init(CajaMenuProviderIface *iface) {
  iface->get_file_items = caja_cwe_get_file_items;
}

static void caja_cwe_instance_init(CajaCwe *cwe) {}

static void caja_cwe_class_init(CajaCweClass *class) {
  parent_class = g_type_class_peek_parent(class);
}

static GType cwe_type = 0;

GType caja_cwe_get_type(void) { return cwe_type; }

void caja_cwe_register_type(GTypeModule *module) {
  static const GTypeInfo info = {sizeof(CajaCweClass),
                                 (GBaseInitFunc)NULL,
  ...
  static const GInterfaceInfo menu_provider_iface_info = {
      (GInterfaceInitFunc)caja_cwe_menu_provider_iface_init, NULL, NULL};

  cwe_type =
      g_type_module_register_type(module, G_TYPE_OBJECT, "CajaCwe", &info, 0);

  g_type_module_add_interface(module, cwe_type, CAJA_TYPE_MENU_PROVIDER,
