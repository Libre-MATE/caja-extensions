...
static gboolean image_converter_file_is_image(CajaFileInfo *file_info) {
  gchar *uri_scheme;
  gchar *mime_type;
  ...
  if (strcmp(uri_scheme, "file") != 0) maybe_image = FALSE;
  g_free(uri_scheme);

  mime_type = caja_file_info_get_mime_type(file_info);
  if (strncmp(mime_type, "image/", 6) != 0) maybe_image = FALSE;
  g_free(mime_type);

  return maybe_image;
}

static GList *image_converter_filter_images(GList *files) {
  GList *images;
  GList *file;
...
static void image_resize_callback(CajaMenuItem *item, GList *files) {
  CajaImageResizer *resizer =
      caja_image_resizer_new(image_converter_filter_images(files));
  caja_image_resizer_show_dialog(resizer);
}

static void image_rotate_callback(CajaMenuItem *item, GList *files) {
  CajaImageRotator *rotator =
      caja_image_rotator_new(image_converter_filter_images(files));
  caja_image_rotator_show_dialog(rotator);
}

static GList *caja_image_converter_get_background_items(
  ...
  for (file = files; file != NULL; file = file->next) {
    if (image_converter_file_is_image(file->data)) {
      item = caja_menu_item_new("CajaImageConverter::resize",
                                _("_Resize Images..."),
                                _("Resize each selected image"), NULL);
      g_signal_connect(item, "activate", G_CALLBACK(image_resize_callback),
      ...
      item = caja_menu_item_new("CajaImageConverter::rotate",
                                _("Ro_tate Images..."),
                                _("Rotate each selected image"), NULL);
      g_signal_connect(item, "activate", G_CALLBACK(image_rotate_callback),
                       caja_file_info_list_copy(files));
...
static void caja_image_converter_menu_provider_iface_init(
...
GType caja_image_converter_get_type(void) { return image_converter_type; }
...
void caja_image_converter_register_type(GTypeModule *module) {
  ...
  static const GInterfaceInfo menu_provider_iface_info = {
      (GInterfaceInitFunc)caja_image_converter_menu_provider_iface_init, NULL,
      NULL};

  image_converter_type = g_type_module_register_type(
      module, G_TYPE_OBJECT, "CajaImageConverter", &info, 0);

  g_type_module_add_interface(module, image_converter_type,
