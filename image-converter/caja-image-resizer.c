...
static void caja_image_resizer_finalize(GObject *object) {
  CajaImageResizer *resizer = CAJA_IMAGE_RESIZER(object);

  g_free(resizer->suffix);

  if (resizer->size) g_free(resizer->size);

  G_OBJECT_CLASS(caja_image_resizer_parent_class)->finalize(object);
}

static void caja_image_resizer_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
  CajaImageResizer *resizer = CAJA_IMAGE_RESIZER(object);
...
static void caja_image_resizer_get_property(GObject *object, guint property_id,
...
static void caja_image_resizer_class_init(CajaImageResizerClass *klass) {
  ...
  files_param_spec =
      g_param_spec_pointer("files", "Files", "Set selected files",
                           G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE);

  g_object_class_install_property(object_class, PROP_FILES, files_param_spec);
}

static void run_op(CajaImageResizer *resizer);

static GFile *caja_image_resizer_transform_filename(CajaImageResizer *resizer,
                                                    GFile *orig_file) {
  ...
  if (extension != NULL) basename[strlen(basename) - strlen(extension)] = '\0';

  new_basename = g_strdup_printf(
      "%s%s%s", basename, resizer->suffix == NULL ? ".tmp" : resizer->suffix,
...
static void op_finished(GPid pid, gint status, gpointer data) {
  ...
  if (status != 0) {
    ...
    builder = gtk_builder_new_from_resource(
        "/org/mate/caja/extensions/imageconverter/error-dialog.ui");
    ...
    GFile *new_location =
        caja_image_resizer_transform_filename(resizer, orig_location);
    g_file_move(new_location, orig_location, G_FILE_COPY_OVERWRITE, NULL, NULL,
                NULL, NULL);
...
static void run_op(CajaImageResizer *resizer) {
  ...
  /* FIXME: check whether new_uri already exists and provide "Replace _All",
   * "_Skip", and "_Replace" options */
...
static void on_caja_image_resizer_response(GtkDialog *dialog, gint response_id,
                                           CajaImageResizer *resizer) {
  if (response_id == GTK_RESPONSE_OK) {
    if (gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(resizer->append_radiobutton))) {
    ...
    if (gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(resizer->default_size_radiobutton))) {
      resizer->size = gtk_combo_box_text_get_active_text(
          GTK_COMBO_BOX_TEXT(resizer->size_combobox));
    } else if (gtk_toggle_button_get_active(
                   GTK_TOGGLE_BUTTON(resizer->custom_pct_radiobutton))) {
      resizer->size = g_strdup_printf(
          "%d%%", gtk_spin_button_get_value_as_int(resizer->pct_spinbutton));
    } else {
      resizer->size = g_strdup_printf(
          "%dx%d", gtk_spin_button_get_value_as_int(resizer->width_spinbutton),
...
static void caja_image_resizer_init(CajaImageResizer *resizer) {
  GtkBuilder *builder;

  builder = gtk_builder_new_from_resource(
      "/org/mate/caja/extensions/imageconverter/caja-image-resize.ui");
#ifdef ENABLE_NLS
  gtk_builder_set_translation_domain(builder, GETTEXT_PACKAGE);
#endif /* ENABLE_NLS */

  /* Grab some widgets */
  resizer->resize_dialog =
      GTK_DIALOG(gtk_builder_get_object(builder, "resize_dialog"));
  resizer->default_size_radiobutton = GTK_RADIO_BUTTON(
      gtk_builder_get_object(builder, "default_size_radiobutton"));
  resizer->size_combobox =
      GTK_COMBO_BOX_TEXT(gtk_builder_get_object(builder, "comboboxtext_size"));
  resizer->custom_pct_radiobutton = GTK_RADIO_BUTTON(
  ...
  resizer->height_spinbutton =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "height_spinbutton"));
  resizer->append_radiobutton =
  ...
  g_signal_connect(resizer->resize_dialog, "response",
                   G_CALLBACK(on_caja_image_resizer_response), resizer);
...
void caja_image_resizer_show_dialog(CajaImageResizer *resizer) {
