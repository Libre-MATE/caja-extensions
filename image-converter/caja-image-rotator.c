...
static void caja_image_rotator_finalize(GObject *object) {
  CajaImageRotator *rotator = CAJA_IMAGE_ROTATOR(object);

  g_free(rotator->suffix);

  if (rotator->angle) g_free(rotator->angle);

  G_OBJECT_CLASS(caja_image_rotator_parent_class)->finalize(object);
}

static void caja_image_rotator_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
  CajaImageRotator *rotator = CAJA_IMAGE_ROTATOR(object);
...
static void caja_image_rotator_get_property(GObject *object, guint property_id,
...
static void caja_image_rotator_class_init(CajaImageRotatorClass *klass) {
...
static GFile *caja_image_rotator_transform_filename(CajaImageRotator *rotator,
                                                    GFile *orig_file) {
  ...
  new_basename = g_strdup_printf(
      "%s%s%s", basename, rotator->suffix == NULL ? ".tmp" : rotator->suffix,
...
static void op_finished(GPid pid, gint status, gpointer data) {
  ...
  if (status != 0) {
    ...
    GFile *new_location =
        caja_image_rotator_transform_filename(rotator, orig_location);
...
static void on_caja_image_rotator_response(GtkDialog *dialog, gint response_id,
                                           CajaImageRotator *rotator) {
  if (response_id == GTK_RESPONSE_OK) {
    if (gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(rotator->append_radiobutton))) {
    ...
    if (gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(rotator->default_angle_radiobutton))) {
      switch (
          gtk_combo_box_get_active(GTK_COMBO_BOX(rotator->angle_combobox))) {
        case 0:
        ...
        default:
          g_assert_not_reached();
                   ...
                   GTK_TOGGLE_BUTTON(rotator->custom_angle_radiobutton))) {
      rotator->angle = g_strdup_printf(
          "%d", gtk_spin_button_get_value_as_int(rotator->angle_spinbutton));
    } else {
      g_assert_not_reached();
    }
...
static void caja_image_rotator_init(CajaImageRotator *rotator) {
  GtkBuilder *builder;

  builder = gtk_builder_new_from_resource(
      "/org/mate/caja/extensions/imageconverter/caja-image-rotate.ui");
#ifdef ENABLE_NLS
  gtk_builder_set_translation_domain(builder, GETTEXT_PACKAGE);
#endif /* ENABLE_NLS */

  /* Grab some widgets */
  rotator->rotate_dialog =
      GTK_DIALOG(gtk_builder_get_object(builder, "rotate_dialog"));
  rotator->default_angle_radiobutton = GTK_RADIO_BUTTON(
      gtk_builder_get_object(builder, "default_angle_radiobutton"));
  rotator->angle_combobox =
      GTK_COMBO_BOX(gtk_builder_get_object(builder, "angle_combobox"));
  rotator->custom_angle_radiobutton = GTK_RADIO_BUTTON(
      gtk_builder_get_object(builder, "custom_angle_radiobutton"));
  ...
  g_signal_connect(rotator->rotate_dialog, "response",
                   G_CALLBACK(on_caja_image_rotator_response), rotator);
...
void caja_image_rotator_show_dialog(CajaImageRotator *rotator) {
