...
static TerminalFileInfo get_terminal_file_info(CajaFileInfo *file_info) {
...
static char *lookup_in_data_dir(const char *basename, const char *data_dir) {
  char *path;
...
static char *lookup_in_data_dirs(const char *basename) {
...
static inline gboolean desktop_opens_home_dir(void) {
  gboolean result;
  GSettings *settings;
...
static inline gboolean set_desktop_opens_home_dir(gboolean val) {
  gboolean result;
  GSettings *settings;
...
static inline gchar *default_terminal_application(void) {
  gchar *result;
  GSettings *settings;
...
static inline gboolean set_default_terminal_application(const gchar *exec) {
  gboolean result;
  GSettings *settings;
...
static void parse_sftp_uri(GFile *file, char **host, guint *port, char **user,
                           char **path) {
...
static void append_sftp_info(char **terminal_exec, CajaFileInfo *file_info) {
  GFile *vfs_uri;
  ...
  cmd = g_strdup_printf("ssh %s %s -t %s", user_host, host_port_switch,
                        quoted_remote_cmd);
  quoted_cmd = g_shell_quote(cmd);
  g_free(cmd);

  *terminal_exec = g_realloc(
      *terminal_exec, strlen(*terminal_exec) + strlen(quoted_cmd) + 4 + 1);
...
static void open_terminal_callback(CajaMenuItem *item,
  ...
  switch (get_terminal_file_info(file_info)) {
    ...
    case FILE_INFO_DESKTOP:
      if (desktop_is_home_dir() || desktop_opens_home_dir()) {
        ...
        working_directory =
            g_strdup(g_get_user_special_dir(G_USER_DIRECTORY_DESKTOP));
      }
      break;
  ...
  if (dfile != NULL) {
    ...
    if (orig_cwd == -1) {
      g_message(
          "CajaOpenTerminal: Failed to open current Caja working directory.");
    } else if (working_directory != NULL) {
      if (chdir(working_directory) == -1) {
        int ret;

        g_message(
            "CajaOpenTerminal: Failed to change Caja working directory to "
            "\"%s\".",
            working_directory);
        ...
        if (ret == -1) {
          g_message(
              "CajaOpenTerminal: Failed to close() current Caja working "
              "directory.");
        }
    ...
    mate_desktop_item_launch(ditem, NULL,
                             MATE_DESKTOP_ITEM_LAUNCH_USE_CURRENT_DIR, NULL);
    ...
    if (orig_cwd != -1) {
      ...
      if (ret == -1) {
        g_message(
            "CajaOpenTerminal: Failed to change back Caja working directory to "
            "original location after changing it to \"%s\".",
            working_directory);
      }
    ...
    g_spawn_async(working_directory, argv, NULL, G_SPAWN_SEARCH_PATH, NULL,
                  NULL, NULL, NULL);
  }
...
static CajaMenuItem *open_terminal_menu_item_new(
    CajaFileInfo *file_info, TerminalFileInfo terminal_file_info,
    GdkScreen *screen, gboolean is_file_item) {
  CajaMenuItem *ret;
  const char *name;
  const char *tooltip;
  ...
  ret = caja_menu_item_new("CajaOpenTerminal::open_terminal", name, tooltip,
                           "terminal");

  g_object_set_data(G_OBJECT(ret), "CajaOpenTerminal::screen", screen);

  g_object_set_data_full(G_OBJECT(ret), "file-info", g_object_ref(file_info),
                         (GDestroyNotify)g_object_unref);
  g_signal_connect(ret, "activate", G_CALLBACK(open_terminal_callback),
                   file_info);

  return ret;
}

static GList *caja_open_terminal_get_background_items(
    CajaMenuProvider *provider, GtkWidget *window, CajaFileInfo *file_info) {
  CajaMenuItem *item;
  ...
  switch (terminal_file_info) {
    ...
    case FILE_INFO_SFTP:
      item = open_terminal_menu_item_new(file_info, terminal_file_info,
                                         gtk_widget_get_screen(window), FALSE);
...
static GList *caja_open_terminal_get_file_items(CajaMenuProvider *provider,
                                                GtkWidget *window,
                                                GList *files) {
  CajaMenuItem *item;
  TerminalFileInfo terminal_file_info;
  ...
  switch (terminal_file_info) {
    case FILE_INFO_LOCAL:
    case FILE_INFO_SFTP:
      item = open_terminal_menu_item_new(files->data, terminal_file_info,
                                         gtk_widget_get_screen(window), TRUE);
      return g_list_append(NULL, item);
...
static void caja_open_terminal_run_config(CajaConfigurable *provider) {
  GtkWidget *extconf_dialog, *extconf_content, *extconf_desktophomedir,
      *extconf_inform1, *extconf_inform2, *extconf_exec;
  gchar *terminal;

  extconf_dialog = gtk_dialog_new();
  extconf_content = gtk_dialog_get_content_area(GTK_DIALOG(extconf_dialog));

  extconf_desktophomedir = gtk_check_button_new_with_label(
      _("Open at Home if trying to open on desktop"));
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(extconf_desktophomedir),
  ...
  gtk_box_set_child_packing(GTK_BOX(extconf_inform1), extconf_exec, FALSE,
                            FALSE, 0, GTK_PACK_END);
  ...
  gtk_widget_show(extconf_exec);
  gtk_dialog_add_buttons(GTK_DIALOG(extconf_dialog), _("Close"),
  ...
  gtk_window_set_title(GTK_WINDOW(extconf_dialog),
                       _("open-terminal Configuration"));
  gtk_dialog_run(GTK_DIALOG(extconf_dialog));

  set_default_terminal_application(gtk_entry_get_text(GTK_ENTRY(extconf_exec)));
  set_desktop_opens_home_dir(
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(extconf_desktophomedir)));

  gtk_widget_destroy(GTK_WIDGET(extconf_dialog));
}

static void caja_open_terminal_menu_provider_iface_init(
...
static void caja_open_terminal_configurable_iface_init(
    CajaConfigurableIface *iface) {
...
GType caja_open_terminal_get_type(void) { return terminal_type; }
...
void caja_open_terminal_register_type(GTypeModule *module) {
  ...
  static const GInterfaceInfo menu_provider_iface_info = {
      (GInterfaceInitFunc)caja_open_terminal_menu_provider_iface_init, NULL,
      NULL};

  static const GInterfaceInfo configurable_iface_info = {
      (GInterfaceInitFunc)caja_open_terminal_configurable_iface_init, NULL,
      NULL};

  terminal_type = g_type_module_register_type(module, G_TYPE_OBJECT,
                                              "CajaOpenTerminal", &info, 0);

  g_type_module_add_interface(module, terminal_type, CAJA_TYPE_MENU_PROVIDER,
                              &menu_provider_iface_info);

  g_type_module_add_interface(module, terminal_type, CAJA_TYPE_CONFIGURABLE,
