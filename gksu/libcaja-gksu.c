...
static void gksu_context_menu_init(GksuContextMenu *self);
static void gksu_context_menu_class_init(GksuContextMenuClass *class);
static void menu_provider_iface_init(CajaMenuProviderIface *iface);

static GList *gksu_context_menu_get_file_items(CajaMenuProvider *provider,
                                               GtkWidget *window, GList *files);
static void gksu_context_menu_activate(CajaMenuItem *item, CajaFileInfo *file);

static GType gksu_context_menu_get_type(void) { return gksucm_type; }

static void gksu_context_menu_register_type(GTypeModule *module) {
  static const GTypeInfo info = {sizeof(GksuContextMenuClass),
                                 (GBaseInitFunc)NULL,
  ...
  gksucm_type = g_type_module_register_type(module, G_TYPE_OBJECT,
                                            "GksuContextMenu", &info, 0);
  g_type_module_add_interface(module, gksucm_type, CAJA_TYPE_MENU_PROVIDER,
...
static GList *gksu_context_menu_get_file_items(CajaMenuProvider *provider,
  ...
  if (files == NULL || files->next != NULL) return NULL;
  ...
  item = caja_menu_item_new("Gksu::open_as_root", _("Open as administrator"),
                            _("Opens the file with administrator privileges"),
                            NULL);
  g_signal_connect_object(item, "activate",
                          G_CALLBACK(gksu_context_menu_activate), file, 0);
...
static void gksu_context_menu_activate(CajaMenuItem *item, CajaFileInfo *file) {
  ...
  if (!strcmp(mime_type,
              "application/x-desktop")) { /* we're handling a .desktop file */
    ...
    GAppInfo *app_info =
        g_app_info_get_default_for_type(mime_type, strncmp(uri, "file://", 7));
    if (app_info) {
