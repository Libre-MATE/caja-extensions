...
static char *hex_unescape_string(const char *str, size_t *out_len,
  ...
  *p++ = 0;
  ...
  if (out_len) *out_len = p - unescaped_str;
  *free_return = TRUE;
...
static gchar *caja_xattr_tags_get_xdg_tags(CajaFileInfo *file) {
  ...
  info = g_file_query_info(location, G_FILE_ATTRIBUTE_XATTR_XDG_TAGS, 0, NULL,
                           NULL);

  if (info) {
    if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_XATTR_XDG_TAGS)) {
      const gchar *escaped_tags = g_file_info_get_attribute_string(
          info, G_FILE_ATTRIBUTE_XATTR_XDG_TAGS);

      gboolean new_created = FALSE;
      gchar *un_escaped_tags =
          hex_unescape_string(escaped_tags, NULL, &new_created);
      if (new_created) {
...
static CajaOperationResult caja_xattr_tags_update_file_info(
...
static void caja_xattr_tags_cancel_update(CajaInfoProvider *provider,
                                          CajaOperationHandle *handle) {
...
static void caja_xattr_tags_info_provider_iface_init(
...
static GList *caja_xattr_tags_get_columns(CajaColumnProvider *provider) {
  GList *ret = NULL;
  CajaColumn *column = NULL;

  column = caja_column_new(XATTR_TAGS_NAME, XATTR_TAGS_ATTRIBUTE, _("Tags"),
...
static void caja_xattr_tags_column_provider_iface_init(
    CajaColumnProviderIface *iface) {
  iface->get_columns = caja_xattr_tags_get_columns;
}

static void caja_xattr_tags_instance_init(CajaXattrTags *cajaXattrTags) {}

static void caja_xattr_tags_class_init(CajaXattrTagsClass *class) {
  parent_class = g_type_class_peek_parent(class);
}

static GType caja_xattr_tags_type = 0;

GType caja_xattr_tags_get_type(void) { return caja_xattr_tags_type; }
...
void caja_xattr_tags_register_type(GTypeModule *module) {
  ...
  caja_xattr_tags_type = g_type_module_register_type(module, G_TYPE_OBJECT,
                                                     "CajaXattrTags", &info, 0);

  static const GInterfaceInfo info_provider_iface_info = {
      (GInterfaceInitFunc)caja_xattr_tags_info_provider_iface_init, NULL, NULL};

  g_type_module_add_interface(module, CAJA_TYPE_XATTR_TAGS,
                              CAJA_TYPE_INFO_PROVIDER,
                              &info_provider_iface_info);

  static const GInterfaceInfo column_provider_iface_info = {
      (GInterfaceInitFunc)caja_xattr_tags_column_provider_iface_init, NULL,
      NULL};
