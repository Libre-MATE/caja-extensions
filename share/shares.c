...
static gboolean net_usershare_run(int argc, char **argv,
                                  GKeyFile **ret_key_file, GError **error) {
  int real_argc;
  int i;
  char **real_argv;
  gboolean retval;
  ...
  if (ret_key_file) *ret_key_file = NULL;
  ...
  */
  ...
  retval = g_spawn_sync(NULL,                      /* cwd */
                        real_argv, NULL,           /* envp */
                        G_SPAWN_SEARCH_PATH, NULL, /* GSpawnChildSetupFunc */
                        NULL,                      /* user_data */
                        &stdout_contents, &stderr_contents, &exit_status,
                        &real_error);

  /* g_message ("returned from spawn: %s: %s", retval ? "SUCCESS" : "FAIL",
   * retval ? "" : real_error->message); */
  ...
  if (!WIFEXITED(exit_status)) {
    ...
    if (WIFSIGNALED(exit_status)) {
      ...
      g_set_error(error, SHARES_ERROR, SHARES_ERROR_FAILED,
                  _("%s %s %s returned with signal %d"), real_argv[0],
                  real_argv[1], real_argv[2], signal_num);
    } else
      g_set_error(error, SHARES_ERROR, SHARES_ERROR_FAILED,
                  _("%s %s %s failed for an unknown reason"), real_argv[0],
                  real_argv[1], real_argv[2]);
  ...
  if (exit_code != 0) {
    ...
    if (str && str[0])
      message = g_strdup_printf(_("'net usershare' returned error %d: %s"),
                                exit_code, str);
    else
      message =
          g_strdup_printf(_("'net usershare' returned error %d"), exit_code);

    g_free(str);

    g_set_error(error, G_SPAWN_ERROR, G_SPAWN_ERROR_FAILED, "%s", message);
  ...
  if (ret_key_file) {
    ...
    *ret_key_file = NULL;
    ...
    /* FIXME: jeallison@novell.com says the output of "net usershare" is nearly
     * always in UTF-8, but that it can be configured in the master smb.conf. We
    ...
    real_error = NULL;
    if (!g_key_file_load_from_data(key_file, stdout_contents, -1, 0,
                                   &real_error)) {
...
static ShareInfo *lookup_share_by_path(const char *path) {
  ensure_hashes();
  return g_hash_table_lookup(path_share_info_hash, path);
}

static ShareInfo *lookup_share_by_share_name(const char *share_name) {
  ensure_hashes();
  return g_hash_table_lookup(share_name_share_info_hash, share_name);
}

static void add_share_info_to_hashes(ShareInfo *info) {
  ensure_hashes();
...
static void remove_share_info_from_hashes(ShareInfo *info) {
  ensure_hashes();
  g_hash_table_remove(path_share_info_hash, info->path);
  g_hash_table_remove(share_name_share_info_hash, info->share_name);
}

static gboolean remove_from_path_hash_cb(gpointer key, gpointer value,
                                         gpointer data) {
  ShareInfo *info;
...
static gboolean remove_from_share_name_hash_cb(gpointer key, gpointer value,
                                               gpointer data) {
  /* The ShareInfo was already freed in remove_from_path_hash_cb() */
  return TRUE;
}

static void free_all_shares(void) {
  ensure_hashes();
  g_hash_table_foreach_remove(path_share_info_hash, remove_from_path_hash_cb,
                              NULL);
  g_hash_table_foreach_remove(share_name_share_info_hash,
                              remove_from_share_name_hash_cb, NULL);
}

static char *get_string_from_key_file(GKeyFile *key_file, const char *group,
                                      const char *key) {
  GError *error;
  char *str;
  ...
  if (g_key_file_has_key(key_file, group, key, &error)) {
    ...
    if (!str) {
      g_assert(!g_error_matches(error, G_KEY_FILE_ERROR,
...
static void add_key_group_to_hashes(GKeyFile *key_file, const char *group) {
  char *path;
  char *comment;
  char *acl;
  ...
  if (!path) {
    g_message("group '%s' doesn't have a '%s' key!  Ignoring group.", group,
              KEY_PATH);
    return;
  }
  ...
  if (acl) {
    if (strstr(acl, "Everyone:R"))
      ...
      g_message(
          "unknown format for key '%s/%s' as it contains '%s'.  Assuming that "
    ...
    g_message(
        "group '%s' doesn't have a '%s' key!  Assuming that the share is "
        "read-only.",
        group, KEY_ACL);
    is_writable = FALSE;
  }
  ...
  if (guest_ok_str) {
    if (strcmp(guest_ok_str, "n") == 0)
      ...
      g_message(
          "unknown format for key '%s/%s' as it contains '%s'.  Assuming that "
          "the share is not guest accessible.",
    ...
    g_message(
        "group '%s' doesn't have a '%s' key!  Assuming that the share is not "
        "guest accessible.",
        group, KEY_GUEST_OK);
    guest_ok = FALSE;
  }
...
static void replace_shares_from_key_file(GKeyFile *key_file) {
  gsize num_groups;
  char **group_names;
  gsize i;
...
static gboolean refresh_shares(GError **error) {
  GKeyFile *key_file;
  char *argv[1];
  GError *real_error;
  ...
  if (throw_error_on_refresh) {
    g_set_error(error, SHARES_ERROR, SHARES_ERROR_FAILED, _("Failed"));
    return FALSE;
  }
  ...
  if (!net_usershare_run(G_N_ELEMENTS(argv), argv, &key_file, &real_error)) {
    g_message("Called \"net usershare info\" but it failed: %s",
              real_error->message);
    g_propagate_error(error, real_error);
    return FALSE;
  }
...
static gboolean refresh_if_needed(GError **error) {
  gboolean retval;
...
static ShareInfo *copy_share_info(ShareInfo *info) {
  ShareInfo *copy;
...
gboolean shares_supports_guest_ok(gboolean *supports_guest_ok_ret,
  ...
  *supports_guest_ok_ret = FALSE;
  ...
  result = g_spawn_command_line_sync(
      "testparm -s --parameter-name='usershare allow guests'", &stdout_contents,
      &stderr_contents, &exit_status, error);
  ...
  if (!WIFEXITED(exit_status)) {
    if (WIFSIGNALED(exit_status)) {
      ...
      signal_num = WTERMSIG(exit_status);
      g_set_error(error, SHARES_ERROR, SHARES_ERROR_FAILED,
                  _("Samba's testparm returned with signal %d"), signal_num);
    } else
      g_set_error(error, SHARES_ERROR, SHARES_ERROR_FAILED,
  ...
  if (exit_code != 0) {
    ...
    if (str && str[0])
      message = g_strdup_printf(_("Samba's testparm returned error %d: %s"),
                                exit_code, str);
    else
      message =
          g_strdup_printf(_("Samba's testparm returned error %d"), exit_code);
  ...
  *supports_guest_ok_ret =
      (g_ascii_strncasecmp(stdout_contents, "Yes", 3) == 0);
...
static gboolean add_share(ShareInfo *info, GError **error) {
  char *argv[7];
  int argc;
  ShareInfo *copy;
  GKeyFile *key_file;
  ...
  if (!supports_success) return FALSE;
  ...
  if (!net_usershare_success) {
    g_message("Called \"net usershare add\" but it failed: %s",
              real_error->message);
    g_propagate_error(error, real_error);
    return FALSE;
  }
...
static gboolean remove_share(const char *path, GError **error) {
  ShareInfo *old_info;
  char *argv[2];
  GError *real_error;
  ...
  if (throw_error_on_remove) {
    g_set_error(error, SHARES_ERROR, SHARES_ERROR_FAILED, "Failed");
  ...
  if (!old_info) {
    ...
    g_set_error(
        error, SHARES_ERROR, SHARES_ERROR_NONEXISTENT,
  ...
  if (!net_usershare_run(G_N_ELEMENTS(argv), argv, NULL, &real_error)) {
    g_message("Called \"net usershare delete\" but it failed: %s",
              real_error->message);
    g_propagate_error(error, real_error);
    g_message("remove_share() end FAIL");
    return FALSE;
...
static gboolean modify_share(const char *old_path, ShareInfo *info,
                             GError **error) {
  ShareInfo *old_info;
  ...
  if (strcmp(info->path, old_info->path) != 0) {
    g_set_error(error, SHARES_ERROR, SHARES_ERROR_FAILED,
                _("Cannot change the path of an existing share; please remove "
                  "the old share first and add a new one"));
  ...
  /* Although "net usershare add" will modify an existing share if it has the
   * same share name as the one that gets passed in, our semantics are
   * different.  We have a one-to-one mapping between paths and share names;
   * "net usershare" supports a one-to-many mapping from paths to share names.
   * So, we must first remove the old share and then add the new/modified one.
   */
  ...
  /* g_message ("modify_share() end: will call add_share() with the new share
   * info"); */
  return add_share(info, error);
}
...
GQuark shares_error_quark(void) {
  ...
  if (quark == 0)
    quark = g_quark_from_string(
        "caja-share-error-quark"); /* not from_static_string since we are a
                                      module */
 ...
 * @ret_is_shared: Location to store result value (#TRUE if the path is shared,
 *#FALSE otherwise)
 * @error: Location to store error, or #NULL.
 *
 ...
 **/
gboolean shares_get_path_is_shared(const char *path, gboolean *ret_is_shared,
                                   GError **error) {
 ...
 * @error: Location to store error, or #NULL.
 *
 * Queries the information for a shared path:  its share name, its read-only
 *status, etc.
 *
 * Return value: #TRUE if the info could be queried successfully, #FALSE
 * otherwise.  If this function returns #FALSE, an error code will be returned
 ...
 **/
gboolean shares_get_share_info_for_path(const char *path,
                                        ShareInfo **ret_share_info,
                                        GError **error) {
  ShareInfo *info;
 ...
 * @ret_exists: Location to store return value; #TRUE if the share name exists,
 *#FALSE otherwise.
 *
...
gboolean shares_get_share_name_exists(const char *share_name,
                                      gboolean *ret_exists, GError **error) {
 ...
 * @error argument, and *@ret_share_info will be set to #NULL.
 **/
gboolean shares_get_share_info_for_share_name(const char *share_name,
                                              ShareInfo **ret_share_info,
                                              GError **error) {
  ShareInfo *info;
 ...
 *
 * Return value: TRUE if the share could be modified, FALSE otherwise.  If this
 *returns FALSE, then the error information will be placed in @error.
 **/
gboolean shares_modify_share(const char *old_path, ShareInfo *info,
                             GError **error) {
  ...
  if (!refresh_if_needed(error)) return FALSE;
...
static void copy_to_slist_cb(gpointer key, gpointer value, gpointer data) {
  ShareInfo *info;
  ShareInfo *copy;
  GSList **list;
 ...
 *
 * Frees a list of #ShareInfo structures as returned by
 *shares_get_share_info_list().
 **/
...
void shares_set_debug(gboolean error_on_refresh, gboolean error_on_add,
                      gboolean error_on_modify, gboolean error_on_remove) {
