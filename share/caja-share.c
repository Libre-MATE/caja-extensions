...
#define NEED_IF_GUESTOK_MASK     \
  (S_IRGRP | S_IXGRP | S_IROTH | \
   S_IXOTH) /* need go+rx for guest enabled usershares */
#define NEED_IF_WRITABLE_MASK \
  (S_IWGRP | S_IWOTH) /* writable usershares need go+w additionally*/
...
typedef struct {
  ...
  GtkWidget *main; /* Widget that holds all the rest.  Its "PropertyPage"
...
static void property_page_validate_fields(PropertyPage *page) {
  const char *name;
...
static gboolean message_confirm_missing_permissions(GtkWidget *widget,
                                                    const char *path,
                                                    mode_t need_mask) {
  GtkWidget *toplevel;
  GtkWidget *dialog;
  ...
  if (!GTK_IS_WINDOW(toplevel)) toplevel = NULL;

  display_name = g_filename_display_basename(path);

  dialog = gtk_message_dialog_new(toplevel ? GTK_WINDOW(toplevel) : NULL, 0,
                                  GTK_MESSAGE_QUESTION, GTK_BUTTONS_NONE,
                                  _("Caja needs to add some permissions to "
                                    "your folder \"%s\" in order to share it"),
                                  display_name);
  ...
  gtk_message_dialog_format_secondary_text(
      ...
      _("The folder \"%s\" needs the following extra permissions for sharing "
        "to work:\n"
        "%s%s%s"
        "Do you want Caja to add these permissions to the folder "
        "automatically?"),
      display_name,
      ...
      (need_mask & (S_IWGRP | S_IWOTH)) ? _("  - write permission by others\n")
                                        : "",
      (need_mask & (S_IXGRP | S_IXOTH))
          ? _("  - execute permission by others\n")
          : "");
  g_free(display_name);
  ...
  gtk_dialog_add_button(GTK_DIALOG(dialog),
                        _("Add the permissions automatically"),
                        GTK_RESPONSE_ACCEPT);
...
static void error_when_changing_permissions(GtkWidget *widget,
                                            const char *path) {
  GtkWidget *toplevel;
  GtkWidget *dialog;
  char *display_name;
  ...
  dialog = gtk_message_dialog_new(
      toplevel ? GTK_WINDOW(toplevel) : NULL, 0, GTK_MESSAGE_ERROR,
      GTK_BUTTONS_OK, _("Could not change the permissions of folder \"%s\""),
      display_name);
  g_free(display_name);
...
static char *get_key_file_path(void) {
  return g_build_filename(g_get_home_dir(), ".mate2",
                          "mate-file-manager-share-modified-permissions", NULL);
}

static void save_key_file(const char *filename, GKeyFile *key_file) {
  char *contents;
  gsize length;
...
static void remove_permissions(const char *path, mode_t need_mask) {
  struct stat st;
  mode_t new_mode;
...
static void remove_from_saved_permissions(const char *path,
                                          mode_t remove_mask) {
  GKeyFile *key_file;
  char *key_file_path;
...
static void restore_write_permissions(const char *path) {
...
static ConfirmPermissionsStatus confirm_sharing_permissions(
    GtkWidget *widget, const char *path, gboolean is_shared, gboolean guest_ok,
    gboolean is_writable) {
  struct stat st;
  mode_t mode, new_mode, need_mask;

  if (!is_shared) return CONFIRM_NO_MODIFICATIONS;

  if (stat(path, &st) != 0)
    return CONFIRM_NO_MODIFICATIONS; /* We'll just let "net usershare" give back
                                        an error if the file disappears */

  new_mode = mode = st.st_mode;

  if (guest_ok) new_mode |= NEED_IF_GUESTOK_MASK;
  if (is_writable) new_mode |= NEED_IF_WRITABLE_MASK;
...
static gboolean property_page_commit(PropertyPage *page) {
  gboolean is_shared;
  ...
  is_shared = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(page->checkbutton_share_folder));

  share_info.path = page->path;
  share_info.share_name =
      (char *)gtk_entry_get_text(GTK_ENTRY(page->entry_share_name));
  share_info.comment =
      (char *)gtk_entry_get_text(GTK_ENTRY(page->entry_share_comment));
  share_info.is_writable = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(page->checkbutton_share_rw_ro));
  share_info.guest_ok = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(page->checkbutton_share_guest_ok));
  ...
  status =
      confirm_sharing_permissions(page->main, page->path, is_shared,
                                  share_info.guest_ok, share_info.is_writable);
  if (status == CONFIRM_CANCEL_OR_ERROR)
    return FALSE; /* the user didn't want us to change his folder's permissions
                   */

  error = NULL;
  retval = shares_modify_share(share_info.path, is_shared ? &share_info : NULL,
                               &error);
  ...
  if (!retval) {
    ...
    if (status == CONFIRM_MODIFIED) restore_saved_permissions(page->path);
  } else {
    property_page_validate_fields(page);
    caja_file_info_invalidate_extension_info(page->fileinfo);
  }

  if (!is_shared) restore_saved_permissions(page->path);
...
/*--------------------------------------------------------------------------*/
static gchar *get_fullpath_from_fileinfo(CajaFileInfo *fileinfo) {
  GFile *file;
  gchar *fullpath;
  ...
  g_assert(
      fullpath != NULL &&
      g_file_is_native(
          file)); /* In the beginning we checked that this was a local URI */
  g_object_unref(file);
...
static void property_page_set_warning(PropertyPage *page) {
  GtkStyleContext *context =
      gtk_widget_get_style_context(GTK_WIDGET(page->entry_share_name));
...
static gboolean property_page_share_name_is_valid(PropertyPage *page) {
  const char *newname;
...
static void property_page_set_controls_sensitivity(PropertyPage *page,
                                                   gboolean sensitive) {
  ...
  if (sensitive) {
    ...
    gtk_widget_set_sensitive(page->checkbutton_share_guest_ok,
                             guest_ok_allowed);
  } else
...
static void property_page_check_sensitivity(PropertyPage *page) {
  ...
  gtk_button_set_label(
      GTK_BUTTON(page->button_apply),
      page->was_initially_shared ? _("Modify _Share") : _("Create _Share"));
}

static void modify_share_name_text_entry(GtkEditable *editable,
                                         gpointer user_data) {
  PropertyPage *page;
...
static void modify_share_comment_text_entry(GtkEditable *editable,
                                            gpointer user_data) {
  PropertyPage *page;
...
/*--------------------------------------------------------------------------*/
static void on_checkbutton_share_folder_toggled(GtkToggleButton *togglebutton,
                                                gpointer user_data) {
  PropertyPage *page;
...
static void on_checkbutton_rw_ro_toggled(GtkToggleButton *togglebutton,
                                         gpointer user_data) {
  PropertyPage *page;
...
static void on_checkbutton_guest_ok_toggled(GtkToggleButton *togglebutton,
                                            gpointer user_data) {
  PropertyPage *page;
...
static void free_property_page_cb(gpointer data) {
  PropertyPage *page;
...
static void button_apply_clicked_cb(GtkButton *button, gpointer data) {
  PropertyPage *page;
...
/*--------------------------------------------------------------------------*/
static PropertyPage *create_property_page(CajaFileInfo *fileinfo) {
  PropertyPage *page;
  GError *error;
  ShareInfo *share_info;
  char *share_name;
...
#endif /* ENABLE_NLS */
  g_assert(gtk_builder_add_from_file(
      page->ui, INTERFACES_DIR "/share-dialog.ui", &error));

  page->main = GTK_WIDGET(gtk_builder_get_object(page->ui, "vbox1"));
  g_assert(page->main != NULL);

  g_object_set_data_full(G_OBJECT(page->main), "PropertyPage", page,
                         free_property_page_cb);

  page->checkbutton_share_folder =
      GTK_WIDGET(gtk_builder_get_object(page->ui, "checkbutton_share_folder"));
  page->box_share_content =
      GTK_WIDGET(gtk_builder_get_object(page->ui, "box_share_content"));
  page->checkbutton_share_rw_ro =
      GTK_WIDGET(gtk_builder_get_object(page->ui, "checkbutton_share_rw_ro"));
  page->checkbutton_share_guest_ok = GTK_WIDGET(
      gtk_builder_get_object(page->ui, "checkbutton_share_guest_ok"));
  page->entry_share_name =
      GTK_WIDGET(gtk_builder_get_object(page->ui, "entry_share_name"));
  page->entry_share_comment =
      GTK_WIDGET(gtk_builder_get_object(page->ui, "entry_share_comment"));
  page->label_status =
      GTK_WIDGET(gtk_builder_get_object(page->ui, "label_status"));
  page->button_cancel =
      GTK_WIDGET(gtk_builder_get_object(page->ui, "button_cancel"));
  page->button_apply =
      GTK_WIDGET(gtk_builder_get_object(page->ui, "button_apply"));
  ...
  g_assert(page->checkbutton_share_folder != NULL &&
           ...
           page->entry_share_comment != NULL && page->label_status != NULL &&
           page->button_cancel != NULL && page->button_apply != NULL);
  ...
  if (free_share_name) g_free(share_name);
  ...
  if (share_info)
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(page->checkbutton_share_folder), TRUE);
  else {
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(page->checkbutton_share_folder), FALSE);
  }

  /* Share name */

  if (g_utf8_strlen(gtk_entry_get_text(GTK_ENTRY(page->entry_share_name)), -1) >
      12)
    property_page_set_warning(page);
  ...
  if (share_info != NULL && share_info->is_writable)
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(page->checkbutton_share_rw_ro), TRUE);
  else
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(page->checkbutton_share_rw_ro), FALSE);

  /* Guest access */
  if (share_info != NULL && share_info->guest_ok)
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(page->checkbutton_share_guest_ok), TRUE);
  else
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(page->checkbutton_share_guest_ok), FALSE);
  ...
  g_signal_connect(page->checkbutton_share_folder, "toggled",
                   G_CALLBACK(on_checkbutton_share_folder_toggled), page);

  g_signal_connect(page->checkbutton_share_rw_ro, "toggled",
                   G_CALLBACK(on_checkbutton_rw_ro_toggled), page);

  g_signal_connect(page->checkbutton_share_guest_ok, "toggled",
                   G_CALLBACK(on_checkbutton_guest_ok_toggled), page);

  g_signal_connect(page->entry_share_name, "changed",
                   G_CALLBACK(modify_share_name_text_entry), page);

  g_signal_connect(page->entry_share_comment, "changed",
                   G_CALLBACK(modify_share_comment_text_entry), page);

  g_signal_connect(page->button_apply, "clicked",
                   G_CALLBACK(button_apply_clicked_cb), page);

  if (share_info != NULL) shares_free_share_info(share_info);
...
static CajaShareStatus get_share_status_and_free_share_info(
    ShareInfo *share_info) {
...
/*--------------------------------------------------------------------------*/
static void get_share_info_for_file_info(CajaFileInfo *file,
                                         ShareInfo **share_info,
...
#define NETWORK_SHARE_PREFIX "network:///share-"
  ...
  if (g_str_has_prefix(uri, NETWORK_SHARE_PREFIX)) {
    ...
    if (!shares_get_share_info_for_share_name(share_name, share_info, NULL)) {
      *share_info = NULL;
      *is_shareable = TRUE; /* it *has* the prefix, anyway... we are just
                               unsynchronized with what mate-vfs thinks */
    } else {
      *is_shareable = TRUE;
    }
  ...
  if (!caja_file_info_is_directory(file)) goto out;

  local_path = g_file_get_path(f);
  if (!local_path || !g_file_is_native(f)) goto out;

  /* FIXME: NULL GError */
  if (!shares_get_share_info_for_path(local_path, share_info, NULL)) goto out;
...
/*--------------------------------------------------------------------------*/
static CajaShareStatus file_get_share_status_file(CajaFileInfo *file) {
  ShareInfo *share_info;
  gboolean is_shareable;

  get_share_info_for_file_info(file, &share_info, &is_shareable);

  if (!is_shareable) return CAJA_SHARE_NOT_SHARED;

  return get_share_status_and_free_share_info(share_info);
}

static CajaOperationResult caja_share_update_file_info(
...
static void caja_share_cancel_update(CajaInfoProvider *provider,
                                     CajaOperationHandle *handle) {
...
static void caja_share_info_provider_iface_init(CajaInfoProviderIface *iface) {
 ...
 * items.
 */
static GList *caja_share_get_property_pages(CajaPropertyPageProvider *provider,
                                            GList *files) {
  PropertyPage *page;
  GList *pages;
  ...
  if (!is_shareable) return NULL;

  page = create_property_page(fileinfo);
  gtk_widget_hide(page->button_cancel);

  if (share_info) shares_free_share_info(share_info);

  pages = NULL;
  np_page = caja_property_page_new("CajaShare::property_page",
                                   gtk_label_new(_("Share")), page->main);
...
/*--------------------------------------------------------------------------*/
static void caja_share_property_page_provider_iface_init(
    CajaPropertyPageProviderIface *iface) {
  iface->get_pages = caja_share_get_property_pages;
}

/*--------------------------------------------------------------------------*/
static void caja_share_instance_init(CajaShare *share) {}

/*--------------------------------------------------------------------------*/
static void caja_share_class_init(CajaShareClass *class) {
...
static void button_cancel_clicked_cb(GtkButton *button, gpointer data) {
  GtkWidget *window;
...
static void share_this_folder_callback(CajaMenuItem *item, gpointer user_data) {
  CajaFileInfo *fileinfo;
  PropertyPage *page;
  GtkWidget *window;
...
static GList *caja_share_get_file_items(CajaMenuProvider *provider,
  ...
  item = caja_menu_item_new("CajaShare::share", _("Sharing Options"),
                            _("Share this Folder"), "folder-remote");
  g_signal_connect(item, "activate", G_CALLBACK(share_this_folder_callback),
                   fileinfo);
  g_object_set_data_full(
      G_OBJECT(item), "files", fileinfo,
...
/*--------------------------------------------------------------------------*/
static void caja_share_menu_provider_iface_init(CajaMenuProviderIface *iface) {
...
static GType caja_share_get_type(void) { return share_type; }

static void caja_share_register_type(GTypeModule *module) {
  static const GTypeInfo info = {sizeof(CajaShareClass),
                                 (GBaseInitFunc)NULL,
  ...
  share_type =
      g_type_module_register_type(module, G_TYPE_OBJECT, "CajaShare", &info, 0);

  static const GInterfaceInfo property_page_provider_iface_info = {
      (GInterfaceInitFunc)caja_share_property_page_provider_iface_init, NULL,
      NULL};

  g_type_module_add_interface(module, share_type,
                              CAJA_TYPE_PROPERTY_PAGE_PROVIDER,
                              &property_page_provider_iface_info);

  static const GInterfaceInfo info_provider_iface_info = {
      (GInterfaceInitFunc)caja_share_info_provider_iface_init, NULL, NULL};

  g_type_module_add_interface(module, share_type, CAJA_TYPE_INFO_PROVIDER,
                              &info_provider_iface_info);

  /* Menu right clik */
  static const GInterfaceInfo menu_provider_iface_info = {
      (GInterfaceInitFunc)caja_share_menu_provider_iface_init, NULL, NULL};

  g_type_module_add_interface(module, share_type, CAJA_TYPE_MENU_PROVIDER,
